"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class viconState(object):
    __slots__ = ["position", "angles", "velocity", "angularRates", "timestamp"]

    __typenames__ = ["double", "double", "double", "double", "int64_t"]

    __dimensions__ = [[3], [3], [3], [3], None]

    def __init__(self):
        self.position = [ 0.0 for dim0 in range(3) ]
        self.angles = [ 0.0 for dim0 in range(3) ]
        self.velocity = [ 0.0 for dim0 in range(3) ]
        self.angularRates = [ 0.0 for dim0 in range(3) ]
        self.timestamp = 0

    def encode(self):
        buf = BytesIO()
        buf.write(viconState._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack('>3d', *self.position[:3]))
        buf.write(struct.pack('>3d', *self.angles[:3]))
        buf.write(struct.pack('>3d', *self.velocity[:3]))
        buf.write(struct.pack('>3d', *self.angularRates[:3]))
        buf.write(struct.pack(">q", self.timestamp))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != viconState._get_packed_fingerprint():
            raise ValueError("Decode error")
        return viconState._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = viconState()
        self.position = struct.unpack('>3d', buf.read(24))
        self.angles = struct.unpack('>3d', buf.read(24))
        self.velocity = struct.unpack('>3d', buf.read(24))
        self.angularRates = struct.unpack('>3d', buf.read(24))
        self.timestamp = struct.unpack(">q", buf.read(8))[0]
        return self
    _decode_one = staticmethod(_decode_one)

    _hash = None
    def _get_hash_recursive(parents):
        if viconState in parents: return 0
        tmphash = (0xee293f5d37d60dd7) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if viconState._packed_fingerprint is None:
            viconState._packed_fingerprint = struct.pack(">Q", viconState._get_hash_recursive([]))
        return viconState._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

